...
class Agent_GraphMedNCA(BaseAgent):
    ...
    def train(self, data_loader, loss_function):
        ...
        save_path = os.path.join(self.experiment.get_from_config('model_path'), 'models', f'epoch_{epoch}.pth')
        torch.save(self.model.state_dict(), save_path)
            ...
            with torch.no_grad():
                for idx in tqdm(test_indices):
                    ...
                    if isinstance(item, tuple) and len(item) == 4:
                        # Format: (image_id, color_image, image, label)
                        img_id, colimg, img, label = item
                    elif isinstance(item, tuple) and len(item) == 3:
                        # Format: (color_image, image, label)
                        colimg, img, label = item
                        img_id = f"image_{idx}"  # Create a default ID if none is provided
                    elif isinstance(item, dict):
                        # Format: {'image': img, 'label': label}
                        colimg = item.get('colimg', None)
                        img = item['image']
                        label = item['label']
                    ...
                    try:
                        ...
                        # Process colimg for visualization
                        if isinstance(colimg, torch.Tensor):
                            colimg = colimg.cpu().numpy()

                        # Convert from CHW to HWC format for matplotlib
                        if len(colimg.shape) == 3 and colimg.shape[0] == 3:  # (3, H, W) -> (H, W, 3)
                            colimg = np.transpose(colimg, (1, 2, 0))
                            
                        # Ensure proper normalization for display if needed
                        if np.max(colimg) <= 1.0:
                            # Already normalized to [0,1] range
                            pass
                        elif np.max(colimg) <= 255:
                            # Normalize to [0,1]
                            colimg = colimg / 255.0
